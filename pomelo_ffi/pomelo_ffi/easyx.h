...
#ifndef WINVER
#define WINVER \
    0x0400  // Specifies that the minimum required platform is Windows 95 and
            // Windows NT 4.0.
#endif

#ifndef _WIN32_WINNT
#define _WIN32_WINNT \
    0x0500  // Specifies that the minimum required platform is Windows 2000
#endif

#ifndef _WIN32_WINDOWS
#define _WIN32_WINDOWS \
    0x0410  // Specifies that the minimum required platform is Windows 98
#endif
...
#define EX_SHOWCONSOLE \
    1  // Maintain the console window when creating a graphics window
...
#define BGR(color) \
    ((((color)&0xFF) << 16) | ((color)&0xFF00FF00) | (((color)&0xFF0000) >> 16))
...
void setfillstyle(int style, long hatch = NULL,
                  IMAGE *ppattern = NULL);  // Set fill style
...
void rectangle(int left, int top, int right,
               int bottom);  // Draw a rectangle without filling
void fillrectangle(int left, int top, int right,
                   int bottom);  // Draw a filled rectangle with a border
void solidrectangle(int left, int top, int right,
                    int bottom);  // Draw a filled rectangle without a border
void clearrectangle(int left, int top, int right,
                    int bottom);  // Clear a rectangular region

void circle(int x, int y, int radius);  // Draw a circle without filling
void fillcircle(int x, int y,
                int radius);  // Draw a filled circle with a border
void solidcircle(int x, int y,
                 int radius);  // Draw a filled circle without a border
void clearcircle(int x, int y, int radius);  // Clear a circular region

void ellipse(int left, int top, int right,
             int bottom);  // Draw an ellipse without filling
void fillellipse(int left, int top, int right,
                 int bottom);  // Draw a filled ellipse with a border
void solidellipse(int left, int top, int right,
                  int bottom);  // Draw a filled ellipse without a border
void clearellipse(int left, int top, int right,
                  int bottom);  // Clear an elliptical region
...
void fillroundrect(
    int left, int top, int right, int bottom, int ellipsewidth,
...
void arc(int left, int top, int right, int bottom, double stangle,
         double endangle);  // Draw an arc
void pie(int left, int top, int right, int bottom, double stangle,
         double endangle);  // Draw a sector without filling
...
void fillpolygon(const POINT *points,
                 int num);  // Draw a filled polygon with a border
void solidpolygon(const POINT *points,
                  int num);  // Draw a filled polygon without a border
void clearpolygon(const POINT *points, int num);  // Clear a polygon region

void polybezier(const POINT *points,
                int num);  // Draw three square Bezier curves
void floodfill(int x, int y, COLORREF color,
               int filltype = FLOODFILLBORDER);  // Fill the area

// Text related functions
void outtextxy(int x, int y,
               LPCTSTR str);  // Output a string at the specified location
void outtextxy(int x, int y,
               TCHAR c);      // Output a char at the specified location
...
int drawtext(LPCTSTR str, RECT *pRect,
             UINT uFormat);  // Output a string in the specified format within
                             // the specified area.
int drawtext(TCHAR c, RECT *pRect,
             UINT uFormat);  // Output a char in the specified format within the
                             // specified area.

// Set current text style.
//		nHeight: The height of the text
//		nWidth: The average width of the character. If 0, the scale is
//adaptive. 		lpszFace: The font name 		nEscapement: The writing angle of the
//string, 0.1 degrees, defaults to 0. 		nOrientation: The writing angle of each
//character, 0.1 degrees, defaults to 0. 		nWeight: The stroke weight of the
//character 		bItalic: Specify whether the font is italic 		bUnderline: Specify
//whether the font is underlined 		bStrikeOut: Specify whether the font has a
//strikeout 		fbCharSet: Specifies the character set 		fbOutPrecision: Specifies the
//output accuracy of the text 		fbClipPrecision: Specifies the clip accuracy of
//the text 		fbQuality: Specifies the output quality of the text 		fbPitchAndFamily:
//Specifies a font family that describes a font in a general way
...
void settextstyle(int nHeight, int nWidth, LPCTSTR lpszFace, int nEscapement,
                  int nOrientation, int nWeight, bool bItalic, bool bUnderline,
                  bool bStrikeOut);


void settextstyle(int nHeight, int nWidth, LPCTSTR lpszFace, int nEscapement,
                  int nOrientation, int nWeight, bool bItalic, bool bUnderline,
                  bool bStrikeOut, BYTE fbCharSet, BYTE fbOutPrecision,
...
void loadimage(
    IMAGE *pDstImg, LPCTSTR pImgFile, int nWidth = 0, int nHeight = 0,
    bool bResize =
        false);  // Load image from a file (bmp/gif/jpg/png/tif/emf/wmf/ico)
void loadimage(IMAGE *pDstImg, LPCTSTR pResType, LPCTSTR pResName,
               int nWidth = 0, int nHeight = 0,
               bool bResize = false);  // Load image from resources
...
void FlushBatchDraw(int left, int top, int right,
                    int bottom);  // Refreshes the undisplayed drawing
void EndBatchDraw();  // End batch drawing mode and refreshes the undisplayed
                      // drawing
void EndBatchDraw(int left, int top, int right,
                  int bottom);  // End batch drawing mode and refreshes the
                                // undisplayed drawing
...
//				WM_LBUTTONDOWN		Left mouse button is
//pressed 				WM_LBUTTONUP		Left mouse button is released 				WM_LBUTTONDBLCLK
//Left mouse button is double-clicked
//				WM_MBUTTONDOWN		Middle mouse button is
//pressed 				WM_MBUTTONUP		Middle mouse button is released 				WM_MBUTTONDBLCLK
//Middle mouse button is double-clicked 				WM_RBUTTONDOWN		Right mouse
//button is pressed 				WM_RBUTTONUP		Right mouse button is released
//				WM_RBUTTONDBLCLK	Right mouse button is
//double-clicked
//
...
//	EX_CHAR		WM_CHAR
//
//	EX_WINDOW	WM_ACTIVATE			The window is activated or
//deactivated 				WM_MOVE				The window has been moved
//				WM_SIZE				The size of window has
//changed
...
struct ExMessage {
    ...
    union {
        ...
        struct {
            ...
            bool lbutton : 1;  // Indicates whether the left mouse button is
                               // pressed
            bool mbutton : 1;  // Indicates whether the middle mouse button is
                               // pressed
            bool rbutton : 1;  // Indicates whether the right mouse button is
                               // pressed
            short x;           // The x-coordinate of the cursor
            short y;           // The y-coordinate of the cursor
            short wheel;  // The distance the wheel is rotated, expressed in
                          // multiples or divisions of 120
        };

        // Data of the key message
        struct {
            BYTE vkcode;    // The virtual-key code of the key
            BYTE scancode;  // The scan code of the key. The value depends on
                            // the OEM
            bool extended : 1;  // Indicates whether the key is an extended key,
                                // such as a function key or a key on the
                                // numeric keypad. The value is true if the key
                                // is an extended key; otherwise, it is false.
            bool prevdown : 1;  // Indicates whether the key is previously up or
                                // down
        };
...
// Message Function
ExMessage getmessage(BYTE filter = -1);  // Get a message until a message is
                                         // available for retrieval
void getmessage(ExMessage *msg,
                BYTE filter = -1);  // Get a message until a message is
                                    // available for retrieval
bool peekmessage(ExMessage *msg, BYTE filter = -1,
                 bool removemsg = true);  // Get a message if any exist,
